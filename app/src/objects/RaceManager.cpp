...
RaceManager::CarObject *RaceManager::EndRace(bool executeCallbacks) {
	...
	end_screen.push_back((new Text("SansSerif", glm::vec3(0.1f, -0.2f, 0.0f), glm::vec2(0.0f)))->SetText("Average speed: " + String::formatDouble(dist / race_time_elapsed, 2) + "m/s")->ChangeSize(0.2));
...
unsigned int RaceManager::getLapCount()
{
	return termination_condition_value;
}
