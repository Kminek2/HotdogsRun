...
{
	antiCollider->obbs[0].sizes = glm::vec3(carSize, gameObject->GetModelMaxDistVert()[1].x - gameObject->GetModelMaxDistVert()[1].y, gameObject->GetModelMaxDistVert()[2].x - gameObject->GetModelMaxDistVert()[2].y) * glm::vec3(2.0f * carMovement->getActSpeed() / carMovement->getMaxSpeed() + 3.0f, 1.45f, 1.4f);
	...
	for (int x = -1; x < 2; ++x) {
		for (int y = -1; y < 2; ++y) {
			for (GameObject* obj : GameObject::chunks[{antiCollider->act_chunk.first + x, antiCollider->act_chunk.second + y}]) {
				if ((obj->surface_type == ALWAYS_COLLIDE || obj == map->GetCheckPoint(thisCar->checkpoint + 1)) && obj != mainCar && obj != gameObject && Collisions::checkCollision(*obj, *antiCollider) && (coll == nullptr || glm::distance(obj->transform->position, gameObject->transform->position) < glm::distance(coll->transform->position, gameObject->transform->position))) {
